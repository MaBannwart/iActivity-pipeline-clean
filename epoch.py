...
def split_into_epochs(sample_values, epoch_time_interval, timestamps=None, relative_to_time=None, return_indices=False):
    ...
    # Determine if timestamps are pandas Timestamps (rather than seconds-since-epoch)
    is_pandasTS = np.issubdtype(timestamps.dtype, pd.Timestamp)
    ...
    test = timestamps
    time_offset = timestamps - np.datetime64(epoch_time_offset)
    epoch_time_index = time_offset // epoch_time_interval
