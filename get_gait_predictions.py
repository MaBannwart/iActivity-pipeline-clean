...
root = r'C:/Users/bannw/Downloads/Axivity_Gait_Posture_Data/time_series_merged/'
...
def main(fn, fn_df, s_setup, out_loc, affected=False, no_inperson_standardization=False, data=None, append=False):
    ...
    if not append:
        ...
        predictions_filtered = filter_transient_predictions(df['gait_prediction'], 2)
        ...
        print(f"Finished computing gait predictions: {fn}.")
        ...
        # fig.savefig(head + "/" + fig_string + '_' + date_string + '_gait_predictions.png')
...
def plot_figure(df, title_string):
    ...
    axes = plt.plot([df.at[0, 'Time'], df.at[1, 'Time']], ['no-gait', 'gait'])
    # Remove dummy data
    [u.remove() for u in axes]

    # Plot predictions
    # plt.plot(df['Time'], df['gait_prediction'], color=cycle[0], label="Raw Predictions")
...
def predict_gait(
    ...
    param_standardization = False
    param_append = False

    # Get gait predictions
    gait_predict = main(out_filename, merged_data, param_sensors, param_output_location,
